"""
微信 Windows 客户端 UI 自动化封装

基于 uiautomation 库，将微信窗口抽象为三个核心区域：
- Navigator: 左侧导航栏（微信/通讯录/收藏等标签页切换）
- Session:   会话列表面板（搜索、会话列表、会话操作）
- Chat:      聊天区域（标题栏、消息列表、输入框、发送）

UI 控件定位信息来自 desktop-ui-inspector 对微信 4.x 版本的实际检查。
"""
