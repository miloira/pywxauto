...
def get_client():
    """获取第一个已连接的 WeixinClient 实例"""
    wx = Weixin()
    if not wx.pids:
        print("错误: 未检测到运行中的微信客户端，请先登录微信", file=sys.stderr)
        sys.exit(1)
    pid = wx.pids[0]
    return wx, pid
...
def cmd_send_text(args):
    """发送文本消息"""
    wx, pid = get_client()
    status = wx.send_text(pid, args.to, args.content, timeout=args.timeout)
    print(f"发送状态: {status}")
...
def cmd_send_file(args):
    ...
    if not os.path.exists(args.file):
        ...
        sys.exit(1)
    wx, pid = get_client()
    status = wx.send_file(pid, args.to, args.file, timeout=args.timeout)
    print(f"发送状态: {status}")
...
def cmd_send_image(args):
    ...
    if not os.path.exists(args.file):
        ...
        sys.exit(1)
    wx, pid = get_client()
    status = wx.send_image(pid, args.to, args.file, timeout=args.timeout)
    print(f"发送状态: {status}")
...
def cmd_send_video(args):
    ...
    if not os.path.exists(args.file):
        ...
        sys.exit(1)
    wx, pid = get_client()
    status = wx.send_video(pid, args.to, args.file, timeout=args.timeout)
    print(f"发送状态: {status}")
...
def cmd_send_at(args):
    """在群聊中 @成员发送消息"""
    wx, pid = get_client()
    status = wx.send_at(pid, args.to, args.content, args.members, timeout=args.timeout)
    print(f"发送状态: {status}")
...
def cmd_send_emotion(args):
    """发送表情"""
    wx, pid = get_client()
    status = wx.send_emotion(pid, args.to, keyword=args.keyword, index=args.index, timeout=args.timeout)
    print(f"发送状态: {status}")
...
def cmd_send_collection(args):
    """发送收藏内容"""
    wx, pid = get_client()
    status = wx.send_collection(pid, args.to, args.keyword, timeout=args.timeout)
    print(f"发送状态: {status}")
...
def cmd_send_card(args):
    """发送名片"""
    wx, pid = get_client()
    result = wx.send_card(pid, args.to, args.share)
...
def cmd_get_contact_profile(args):
    """获取联系人资料"""
    wx, pid = get_client()
    profile = wx.get_contact_profile(pid, args.nickname)
...
def cmd_set_contact_remark(args):
    """设置联系人备注"""
    wx, pid = get_client()
    wx.set_contact_remark(pid, args.nickname, args.remark)
...
def cmd_add_contact_label(args):
    """添加联系人标签"""
    wx, pid = get_client()
    wx.add_contact_label(pid, args.nickname, args.labels)
...
def cmd_remove_contact_label(args):
    """移除联系人标签"""
    wx, pid = get_client()
    wx.remove_contact_label(pid, args.nickname, args.labels)
...
def cmd_set_contact_star(args):
    """设为星标朋友"""
    wx, pid = get_client()
    wx.set_contact_star(pid, args.nickname)
...
def cmd_cancel_contact_star(args):
    """取消星标朋友"""
    wx, pid = get_client()
    wx.cancel_contact_star(pid, args.nickname)
...
def cmd_black_contact(args):
    """加入黑名单"""
    wx, pid = get_client()
    wx.black_contact(pid, args.nickname)
...
def cmd_unblack_contact(args):
    """移出黑名单"""
    wx, pid = get_client()
    wx.unblack_contact(pid, args.nickname)
...
def cmd_delete_contact(args):
    """删除联系人"""
    wx, pid = get_client()
    wx.delete_contact(pid, args.nickname)
...
def cmd_create_room(args):
    ...
    if len(args.members) < 2:
        ...
        sys.exit(1)
    wx, pid = get_client()
    wx.create_room(pid, args.members)
...
def cmd_set_room_name(args):
    """设置群聊名称"""
    wx, pid = get_client()
    wx.set_room_name(pid, args.nickname, args.name)
...
def cmd_set_room_announcement(args):
    """设置群公告"""
    wx, pid = get_client()
    wx.set_room_announcement(pid, args.nickname, args.content)
...
def cmd_add_room_members(args):
    """添加群成员"""
    wx, pid = get_client()
    wx.add_room_members(pid, args.nickname, args.members)
...
def cmd_remove_room_members(args):
    """移除群成员"""
    wx, pid = get_client()
    wx.remove_room_members(pid, args.nickname, args.members)
...
def cmd_exit_room(args):
    """退出群聊"""
    wx, pid = get_client()
    wx.exit_room(pid, args.nickname)
...
def cmd_unpin_chat(args):
    """取消置顶"""
    wx, pid = get_client()
    wx.unpin_chat(pid, args.nickname)
...
def cmd_mute_chat(args):
    """消息免打扰"""
    wx, pid = get_client()
    wx.mute_chat(pid, args.nickname)
...
def cmd_unmute_chat(args):
    """取消免打扰"""
    wx, pid = get_client()
    wx.unmute_chat(pid, args.nickname)
...
def cmd_get_moments(args):
    """获取朋友圈动态"""
    wx, pid = get_client()
    moments = wx.get_moments(pid, count=args.count, position=args.position)
    results = []
    for m in moments:
        item = {}
...
def cmd_create_note(args):
    """创建笔记"""
    wx, pid = get_client()
    wx.create_note(pid, args.content)
    print("笔记创建成功")
...
def cmd_get_self_profile(args):
    """获取当前登录账号资料"""
    wx, pid = get_client()
    profile = wx.get_self_profile(pid)
    print(json.dumps(profile, ensure_ascii=False, indent=2))
